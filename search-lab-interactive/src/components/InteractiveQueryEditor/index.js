...
const InteractiveQueryEditor = ({
  ...
  const defaultQueryExecute = async (query, callback) => {
    try {
      ...
      if (errorMsg.includes('Not connected to database')) {
        callback(null, 'Please connect to MongoDB first. Make sure your .env file is configured.');
      } else {
  ...
  const validateResult = (actual, expected) => {
    ...
    } else if (expected.type === 'minCount') {
      return Array.isArray(actual) && actual.length >= expected.value;
    } else if (expected.type === 'hasScore') {
      return Array.isArray(actual) && actual.length > 0 && 
             actual.every(doc => doc.score !== undefined);
    }
    return true;
  };
  ...
  return (
    <div className={styles.queryEditor}>
      ...
      <div className={styles.controls}>
        ...
        >
          {loading ? 'Executing...' : 'Run Search Query'}
        </button>
